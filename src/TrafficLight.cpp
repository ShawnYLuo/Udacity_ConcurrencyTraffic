...
void TrafficLight::waitForGreen()
{
  ...
  while(true){
    TrafficLightPhase tlp = queue.receive();
...
void TrafficLight::simulate()
{
  ...
  TrafficObject::threads.emplace_back(std::move(t));
}
...
void TrafficLight::cycleThroughPhases()
{
  ...
  std::mt19937 eng(rd());
  std::uniform_real_distribution<double> distribution(4000.0,6000.0);
  double r = distribution(eng);
  auto start = std::chrono::system_clock::now();
  while (true){
    
    auto cur = std::chrono::system_clock::now();
    if (std::chrono::duration_cast<std::chrono::milliseconds>(cur-start).count()>r){
